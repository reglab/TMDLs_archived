...
# 
# Note: this script only pulls assessments which resulted in an IR determination
# of category 5, which is defined by the EPA as: "Available data and/or information
# indicate that at least one designated use is not being
...
for (state in states) {
  ...
  for (year in c('2002', '2004', '2006', '2008', '2010', '2012', '2014',
    ...
    # We want assessments that resulted in an IR category of 5 (needing TMDL)
    raw <- fromJSON(paste('https://attains.epa.gov/attains-public/api/assessments?irCategory=5&state=',
    ...
    if (raw$count != 0){
      ...
      # so we unnest them
      state_year_data <- unnest(unnest(unnest(raw$items, assessments, names_repair='universal'), useAttainments, names_repair='universal'), parameters, names_repair='universal')
      
      # In addition, some nested dataframes are mostly unneeded information, 
      # so we can extract the variables directly
      if ((typeof(state_year_data$assessmentMetadata) == 'list') &&
      ...
      state_data <- rbind(state_data, state_year_data)
    }
  }
  write.csv(state_data, paste(state, '_IR5_assessments.csv', sep=""))
  data <- rbind(data, state_data)
  print(paste(state, 'done'))
}
write.csv(data, 'all_IR5_assessments.csv')
